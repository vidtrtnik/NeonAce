...
function generateBuildings() {
    for (var i = 0; i < Math.floor(5 + Math.random() * 15); i++) {
        r = Math.random() * 3;

        var building;

        if (r <= 1.5)
            building = new wy3d_Object("building" + String(i), m_building1, t_building1, 35 + groundObjectsPos[0] + Math.random() * 25, groundObjectsPos[1], -12.0 + groundObjectsPos[2] + Math.random() * 25, 90, 0, 0);
        else
            building = new wy3d_Object("building" + String(i), m_building2, t_building2, 35 + groundObjectsPos[0] + Math.random() * 25, groundObjectsPos[1], -12.0 + groundObjectsPos[2] + Math.random() * 25, 90, 0, 0);
